...
import { getResidentDisplayName } from "../utils/residentProfile";
      ...
      <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="glass-container mt-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between p-6 border-b border-slate-200/50">
            <div>
              <h2 className="text-3xl font-black text-slate-800">Announcement Center</h2>
              <p className="mt-1 text-sm font-medium text-slate-500">
                Publish updates for residents, programs, advisories, and community activities.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-5 p-6 border-b border-slate-200/50 bg-slate-50/20">
            {[
              ["Total", stats.total, Megaphone, "bg-slate-100 text-slate-600"],
              ["Published", stats.published, Send, "bg-emerald-100 text-emerald-600"],
              ["Drafts", stats.drafts, Edit2, "bg-amber-100 text-amber-600"],
              ["Archived", stats.archived, Archive, "bg-slate-100 text-slate-600"],
              ["SMS Target", stats.householdSms, Phone, "bg-blue-100 text-blue-600"],
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <label className="text-sm font-bold text-slate-700 sm:col-span-2">
              ...
              <input
                ...
                className="mt-2 w-full h-[46px] rounded-[12px] border border-slate-200 bg-slate-50 px-4 text-sm font-medium outline-none transition focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 shadow-sm"
              />
            </label>
            ...
            <div className="sm:col-span-2">
              <label className="text-sm font-bold text-slate-700">
                Demo SMS recipient phone numbers
                <textarea
                  name="sms_recipient_phones"
                  value={formData.sms_recipient_phones}
                  onChange={handleInput}
                  rows="3"
                  placeholder={"+639171234567\n+639181234567"}
                  className="mt-2 w-full rounded-[12px] border border-slate-200 bg-slate-50 p-4 text-sm font-medium outline-none transition focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 shadow-sm"
                />
              </label>
              <div className="mt-2 flex flex-col gap-2 text-xs font-semibold text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                <span>
                  {formSmsRecipients.recipients.length} valid recipient(s)
                  {formSmsRecipients.invalid.length > 0
                    ? `, ${formSmsRecipients.invalid.length} invalid`
                    : ""}
                  . Only these numbers receive SMS.
                </span>
              </div>
            </div>
